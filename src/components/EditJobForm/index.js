...
class EditJobForm extends Component {
  ...
  getJobDetails = async () => {
    ...
    this.setState({
      userId: fethcedData.user_id,
      companyName: fethcedData.company_name,
      companyDescription: fethcedData.company_description,
      companyLocation: fethcedData.company_location,
      numberOfPostings: fethcedData.number_of_postings,
      jobTitle: fethcedData.job_title,
      jobDescription: fethcedData.job_description,
      roleCategory: fethcedData.role_category,
      salary: fethcedData.salary,
      workExperience: fethcedData.work_experience,
      activeJobTypeId: fethcedData.job_type,
      educationLevelId: fethcedData.education_level,
      selectedSkills: fethcedData.skills,
    });
  };
  ...
  submitForm = async (event) => {
    ...
    if (this.validateForm()) {
      ...
      const response = await fetch(`http://localhost:3004/jobs/${id}`, options);
      if (response.ok === true) {
